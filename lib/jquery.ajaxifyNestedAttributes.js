...
/*
  ...
  This action should respond to JSON, and provide a response in the following format:
    ...
    {"fields":[{"name":"name","type":"text","value":"some text"},{"name":"body","type":"textarea"}],
      "model":"post","nested_model":"comments"}
  
  fields : Array : list of fields to add to form
    name : the name to apply to the new form element
    type : text | password | textarea
    value : optional value to apply to new from element
  model : a string representation of the model, eg "post" for Post, "comment" for Comment
  nested_model : a string representing the relationship, eg "comments" for 'has_many :comments',
                 "author" for 'has_one :author'
*/
(function($){
  jQuery.ajaxifyNestedAttributes = {
    util : {
      ...
      _add_to_dom : function(json, target, method, options){
        ...
        jQuery.each(json.fields, function(i,field){
          var tag = field['type'];
          var name = field['name'];
          ...
          jQuery(el).attr('name', _name+"["+name+"]").attr('id', _id+'_'+name).appendTo(wrapper);
          if (value != undefined) tag=='textarea' ? jQuery(el).html(value) : jQuery(el).val(value);
          if (options['label'] == true){
            var label = jQuery(document.createElement('label')).attr('for', _id+"_"+name).html(name).insertBefore(el);
          }
        });
        // add a delete field
      ...
      _delete : function(el, method, parent, options){
        ...
        wrapper.find('input').each(function(){
          if (jQuery(this).attr('name').match(/_destroy/)) {
            this.value = 1;
            return false;
          };
        });
